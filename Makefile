...
test: FORCE lint dirs
	...
	RESULTS=temp.results.test python cleanup.py --custom-config -n 1 -v  # from scratch
	RESULTS=temp.results.test python cleanup.py --custom-config -n 1 -v  # use cache
	RESULTS=temp.results.test python evaluate.py -v
